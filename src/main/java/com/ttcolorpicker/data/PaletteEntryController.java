...
public class PaletteEntryController {
	...
	public void NewPaletteEntry(String PaletteName, boolean FillWithRandomValues) {
		PaletteEntry item = new PaletteEntry();
		item.Name = PaletteName;
		if (FillWithRandomValues) {
			for (int i = 0; i < item.Colors.length; i++)
				item.Colors[i] = Color.rgb(random.nextInt(0xff), random.nextInt(0xff), random.nextInt(0xff));
		} else {
			item.ClearColors();
		}
		Palettes.add(item);
